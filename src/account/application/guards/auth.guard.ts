...
export class AuthGuard implements CanActivate {
    ...
    private validatePublicRoles(context: ExecutionContext): { isPublic: boolean, roles: string[] } {
        ...
        if (isPublic) {
            if (roles && _.intersection(roles, defaultRoles).length > 0) { return { isPublic: false, roles } }
